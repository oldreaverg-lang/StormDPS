...
@app.get("/storms")
async def serve_storms_index():
    """Full crawlable index of every /storm/{id} page, grouped by season.

    Generated by scripts/gen_storms_index.py at bake time. Exists because GSC
    showed ~210 of 223 storm pages were orphans — reachable only via sitemap.xml,
    which is a discovery hint, not the internal-link signal Google actually
    allocates crawl budget on."""
    fp = FRONTEND_DIR / "storms.html"
    if not fp.exists():
        raise HTTPException(status_code=404, detail="Not found")
    return FileResponse(
        fp,
