...
No `<script src>` anywhere references it. The same `BASIN_COEFFICIENTS`/`applyBasinDPS`/`window.BasinDPS` logic is **inlined** in index.html (~line 880), and the two copies have drifted (external: EP `ri_bonus: 15`; inline: `ri_bonus:0` everywhere). The compile-cache-bake skill even lists this file as something to keep in sync — a trap for future changes. **Fix:** delete the file (inline copy is canonical) and remove the mention from the bake skill. *(2026-08-14: its `detectBasin` EP-box longitude bound was synced to the inline copy for CP support rather than deleting — deletion is still the right call, deferred to keep the CP change in scope.)*

### A5. JTWC central-Pacific storms get an `EP`-prefixed id → dedupe miss (found 2026-08-14)
`services/jtwc_client.py:282` `suffix_map` maps ATCF suffix `"C"` (central Pacific) to `"EP"`, so a CPHC storm surfacing in JTWC's feed is built as e.g. `EP012026`. NHC's `CurrentStorms.json` calls the same storm `cp012026`. `noaa_client.get_active_storms` (~line 615) dedupes by **exact id**, so the two would not collapse — the same storm could list twice in the active feed. Not currently triggered (JTWC rarely carries a storm NHC/CPHC also has), and unrelated to the CP catalog/scoring fix shipped 2026-08-14. **Fix:** map `"C" → "CP"` in `suffix_map`, or dedupe by (basin-number, year) instead of exact id. Verify no other code keys off the `EP`-from-C behavior first.
