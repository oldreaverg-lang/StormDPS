...
_SURGE_REFERENCE = {
    ...
    "Charley (2004)": (130, 941, 7.0,  "Punta Gorda, FL"),
}
_SURGE_TOLERANCE = 0.35


def validate_surge_model() -> list[str]:
    """
    Quick startup check: compare estimated surge against known historical peaks.
    Returns a list of warning strings (empty = all good).
    Logs a calibration table to stdout regardless.
    """
    warnings = []
    lines = []
    for name, (wind_kt, pressure_mb, observed_ft, location) in _SURGE_REFERENCE.items():
        ...
        if abs(pct_err) > _SURGE_TOLERANCE:
            warnings.append(
                ...
                f"model {modeled_ft:.1f} ft ({pct_err:+.0%}) exceeds "
                f"±{_SURGE_TOLERANCE:.0%} tolerance. Check surge_model.py."
            )
